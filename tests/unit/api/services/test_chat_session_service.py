...
class TestChatSessionServiceProcessing:
    ...
    async def test_process_loads_history_once(self, chat_service, sample_chat_id):
        ...
        await chat_service.process_user_message(sample_chat_id, "Message")

        # Verify history was loaded
        assert chat_service.message_service.get_chat_history.call_count == 1

        # Second message - should use cache
        await chat_service.process_user_message(sample_chat_id, "Message 2")
    ...
    async def test_process_updates_history_cache(
        ...
        # Process message
        await chat_service.process_user_message(sample_chat_id, "Hello")
...
class TestChatSessionServiceAgentIntegration:
    ...
    async def test_generate_response_converts_messages(self, chat_service, mock_user_message):
        ...
        # Generate response
        result = await chat_service._generate_response(history, user_message)
        ...
        # Check result
        assert result == "I'm great!"

    async def test_generate_response_with_empty_history(self, chat_service):
        ...
        result = await chat_service._generate_response([], user_message)
    ...
    async def test_generate_response_preserves_agent_errors(self, chat_service):
        ...
        assert "Invalid OpenAI API key" in result
...
class TestChatSessionServiceIntegration:
    ...
    async def test_full_conversation_flow(self, chat_service, sample_chat_id):
        """Test complete conversation flow with multiple messages."""
        messages = []

        def create_message_side_effect(chat_id, role, content):
            msg = AsyncMock()
            msg.chat_id = chat_id
            msg.role = role
            msg.content = content
        ...
        for i, user_content in enumerate(["Hello", "How are you?", "Bye"]):
            result = await chat_service.process_user_message(sample_chat_id, user_content)
    ...
    async def test_multiple_chats_separate_caches(self, chat_service):
        ...
        await chat_service.process_user_message(chat_id_1, "Hello")

        # Process message in chat 2
        await chat_service.process_user_message(chat_id_2, "Hello")
